...
```
10.1、设置vue-cli的代理，jsonp已经不能完全满足数据的抓取，设置config/index.js中的proxyTable{}；
10.2、获取排行播放列表，并将列表以scroll的形式展示；
10.3、配置vuex，使得播放列表页面的数据与详情页面中交互；
10.4、创建排行播放详情页面，配置子路由，获取详情数据；
10.5、与歌手详情页面类似，设置详情页面，封装歌曲列表数据，方便vuex传递给全局播放控件；
10.6、使用mixin修复播放器遮挡页面的问题；
```

>11、搜索模块实现

```
11.1、开发公共组件searchbar，实现基本的搜索框功能，将搜索关键字实时传送到外部；
11.2、搜索结果列表实现，点击事件分为歌手与歌曲，歌手则打开歌手详情页面，歌曲则将当前列表中所有歌曲添加到播放列表，并播放当前歌曲；
11.3、[热搜与搜索历史]与[搜索结果]为互斥页面，通过绝对布局交替显示；
11.3.1、热搜在页面加载后请求网络交展示，使用flex布局展示；
11.3.2、搜索历史通过vuex及localstorage实现本地化，记录搜索过的关键字；
11.4、开发公共组件confirm，在清除所有搜索记录等危险性操作时先弹框提示；
```
